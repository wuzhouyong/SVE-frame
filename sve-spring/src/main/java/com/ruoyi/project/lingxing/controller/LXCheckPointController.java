...
@RestController
@RequestMapping("/lingxing/checkpoint")
public class LXCheckPointController extends BaseController {

    @Autowired
    private LXCheckPointService checkPointService;


    @PreAuthorize("@ss.hasPermi('lingxing:checkpoint:add')")
    @Log(title = "监测点新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody LXCheckPointEntity entity) {
    ...
    public TableDataInfo list(@RequestBody LXCheckPointEntity entity) {
        startPage();
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<LXCheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        return getDataTable(list);
    }

    @PostMapping("/dropdown")
    public ResultEntity cpDropdown(@RequestBody LXCheckPointEntity entity) {
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<LXCheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        List<Map<String, Object>> mapList = new ArrayList<>();
        for (LXCheckPointEntity item : list) {
            Map<String, Object> map = new LinkedMap<>();
            map.put("value", item.getCollectId());
            map.put("text", item.getCpName());
            mapList.add(map);
        }
        return ResultEntity.success(mapList);
    }

    @PreAuthorize("@ss.hasPermi('lingxing:checkpoint:edit')")
    @Log(title = "监测点编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody LXCheckPointEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(checkPointService.updateCheckPoint(entity));
    }

    @PreAuthorize("@ss.hasPermi('lingxing:checkpoint:del')")
    ...
    public TableDataInfo cpList(@PathVariable String sourceId) {
        ...
        startPage();
        List<LXNameIdEntity> list = checkPointService.selectCPList(sourceId);
        return getDataTable(list);
    }
