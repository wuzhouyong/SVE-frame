...
@RestController
@RequestMapping("/lingxing/video")
public class LXVideoController extends BaseController {

    @Autowired
    private LXVideoService videoService;
    ...
    public TableDataInfo videotype() {
        List<LXNameIdEntity> list = videoService.selectVideoType();
        return getDataTable(list);
    }

    @PostMapping("/list")
    @DataScope(regionAlias = "s")
    public TableDataInfo list(@RequestBody LXVideoEntity entity) {
        startPage();
        ...
        List<LXVideoEntity> list = videoService.selectVideo(entity);
        return getDataTable(list);
    }
    ...
    @PostMapping("/add")
    public AjaxResult add(@RequestBody LXVideoEntity entity) {
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
    ...
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody LXVideoEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
    ...
    public ResultStr getVideoWS(String id, Date time) {
        LXVideoEntity model = videoService.videoModel(id);
        if (model != null) {
            Long start = 0L;
            Long end = 0L;
            if (time != null) {
