...
@RestController
@RequestMapping("/jiayou/video")
public class JYVideoController extends BaseController {

    @Autowired
    private JYVideoService videoService;
    ...
    public TableDataInfo videotype() {
        List<JYNameIdEntity> list = videoService.selectVideoType();
        return getDataTable(list);
    }

    @PostMapping("/list")
    @DataScope(regionAlias = "s")
    public TableDataInfo list(@RequestBody JYVideoEntity entity) {
        startPage();
        ...
        List<JYVideoEntity> list = videoService.selectVideo(entity);
        return getDataTable(list);
    }

    @PreAuthorize("@ss.hasPermi('jiayou:video:add')")
    @Log(title = "视频新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody JYVideoEntity entity) {
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
    ...
    @PreAuthorize("@ss.hasPermi('jiayou:video:edit')")
    @Log(title = "视频编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody JYVideoEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(videoService.updateVideo(entity));
    }

    @PreAuthorize("@ss.hasPermi('jiayou:video:del')")
    ...
    public ResultStr getVideoWS(String id, Date time) {
        JYVideoEntity model = videoService.videoModel(id);
        if (model != null) {
            Long start = 0L;
            Long end = 0L;
            if (time != null) {
