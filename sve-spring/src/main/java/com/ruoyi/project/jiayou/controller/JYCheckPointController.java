...
@RestController
@RequestMapping("/jiayou/checkpoint")
public class JYCheckPointController extends BaseController {

    @Autowired
    private JYCheckPointService checkPointService;


    @PreAuthorize("@ss.hasPermi('jiayou:checkpoint:add')")
    @Log(title = "监测点新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody JYCheckPointEntity entity) {
    ...
    public TableDataInfo list(@RequestBody JYCheckPointEntity entity) {
        startPage();
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<JYCheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        return getDataTable(list);
    }

    @PreAuthorize("@ss.hasPermi('jiayou:checkpoint:edit')")
    @Log(title = "监测点编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody JYCheckPointEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(checkPointService.updateCheckPoint(entity));
    }

    @PreAuthorize("@ss.hasPermi('jiayou:checkpoint:del')")
    ...
    public TableDataInfo cpList(@PathVariable String sourceId) {
        ...
        startPage();
        List<JYNameIdEntity> list = checkPointService.selectCPList(sourceId);
        return getDataTable(list);
    }
