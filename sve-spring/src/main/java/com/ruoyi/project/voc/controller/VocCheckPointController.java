...
@RestController
@RequestMapping("/voc/checkpoint")
public class VocCheckPointController extends BaseController {

    @Autowired
    private VocCheckPointService checkPointService;


    @PreAuthorize("@ss.hasPermi('voc:checkpoint:add')")
    @Log(title = "监测点新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody VocCheckPointEntity entity) {
    ...
    public TableDataInfo list(@RequestBody VocCheckPointEntity entity) {
        startPage();
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<VocCheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        return getDataTable(list);
    }

    @PreAuthorize("@ss.hasPermi('voc:checkpoint:edit')")
    @Log(title = "监测点编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody VocCheckPointEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(checkPointService.updateCheckPoint(entity));
    }

    @PreAuthorize("@ss.hasPermi('voc:checkpoint:del')")
    ...
    public TableDataInfo cpList(@PathVariable String sourceId) {
        ...
        startPage();
        List<VocNameIdEntity> list = checkPointService.selectCPList(sourceId);
        return getDataTable(list);
    }
