...
@RestController
@RequestMapping("/voc/video")
public class VocVideoController extends BaseController {

    @Autowired
    private VocVideoService videoService;
    ...
    public TableDataInfo videotype() {
        List<VocNameIdEntity> list = videoService.selectVideoType();
        return getDataTable(list);
    }

    @PostMapping("/list")
    @DataScope(regionAlias = "s")
    public TableDataInfo list(@RequestBody VocVideoEntity entity) {
        startPage();
        ...
        List<VocVideoEntity> list = videoService.selectVideo(entity);
        return getDataTable(list);
    }
    ...
    @PostMapping("/add")
    public AjaxResult add(@RequestBody VocVideoEntity entity) {
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
    ...
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody VocVideoEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
    ...
    public ResultStr getVideoWS(String id, Date time) {
        VocVideoEntity model = videoService.videoModel(id);
        if (model != null) {
            Long start = 0L;
            Long end = 0L;
            if (time != null) {
            ...
            switch (model.getVideoType()) {
                case "twAgent": {
                    ...
                    String ws = "ws://192.168.10.209:888/byip?ip=" + model.getVideoAddress() + "&user="
                            ...
                            + settings + "&vodstart=" + stamp + "&vodend=0";
