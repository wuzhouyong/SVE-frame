...
import com.ruoyi.project.guocheng.mapper.GCGatherMapper;
...
public class GCGatherServiceImpl implements GCGatherService {

    @Autowired
    private GCGatherMapper gatherMapper;

    @Override
    public List<GCExportGatherEntity> getEnterpriseList(GCGatherEntity entity) {
        return gatherMapper.getEnterpriseList(entity);
    }

    @Override
    public List<GCExportGatherEntity> getTownAbnormalList(GCGatherEntity entity) {
        return gatherMapper.getTownAbnormalList(entity);
    }

    @Override
    public List<GCExportGatherEntity> industrialWastewater(GCGatherEntity entity) {
        List<GCExportGatherEntity> list = gatherMapper.industrialWastewater(entity);
        list.forEach(x -> {
            if (x.getIsExceed() != null) { //解决空指针异常
                switch (x.getIsExceed()) {
                    case 0:
                        x.setExceedName("否");
                        break;
                    case 1:
    ...
    @Override
    public List<GCSourceAbnorEntity> getSourceAbnor(GCSourceAbnorEntity entity) {
