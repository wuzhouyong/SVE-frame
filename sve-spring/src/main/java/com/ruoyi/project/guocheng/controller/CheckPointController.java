...
@RestController
@RequestMapping("/guocheng/checkpoint")
public class CheckPointController extends BaseController {
    ...
    @PreAuthorize("@ss.hasPermi('guocheng:checkpoint:add')")
    @Log(title = "监测点新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody CheckPointEntity entity) {
    ...
    public TableDataInfo list(@RequestBody CheckPointEntity entity) {
        startPage();
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<CheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        return getDataTable(list);
    }

    @PostMapping("/dropdown")
    public ResultEntity cpDropdown(@RequestBody CheckPointEntity entity) {
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<CheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        List<Map<String, Object>> mapList = new ArrayList<>();
        for (CheckPointEntity item : list) {
    ...
    @PreAuthorize("@ss.hasPermi('guocheng:checkpoint:edit')")
    @Log(title = "监测点编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody CheckPointEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(checkPointService.updateCheckPoint(entity));
    }

    @PreAuthorize("@ss.hasPermi('guocheng:checkpoint:del')")
    ...
    public TableDataInfo cpList(@PathVariable String sourceId) {
        ...
        startPage();
        List<GCNameIdEntity> list = checkPointService.selectCPList(sourceId);
        return getDataTable(list);
    }
