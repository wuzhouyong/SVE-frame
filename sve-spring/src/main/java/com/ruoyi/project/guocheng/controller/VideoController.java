...
public class VideoController extends BaseController {
    ...
    public TableDataInfo videotype() {
        List<GCNameIdEntity> list = videoService.selectVideoType();
        return getDataTable(list);
    }

    @PostMapping("/list")
    @DataScope(regionAlias = "s")
    public TableDataInfo list(@RequestBody GCVideoEntity entity) {
        startPage();
        ...
        List<GCVideoEntity> list = videoService.selectVideo(entity);
        return getDataTable(list);
    }

    @PreAuthorize("@ss.hasPermi('guocheng:video:add')")
    @Log(title = "视频新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody GCVideoEntity entity) {
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
    ...
    @PreAuthorize("@ss.hasPermi('guocheng:video:edit')")
    @Log(title = "视频编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody GCVideoEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(videoService.updateVideo(entity));
    }

    @PreAuthorize("@ss.hasPermi('guocheng:video:del')")
    ...
    public ResultStr getVideoWS(String id, Date time) {
        GCVideoEntity model = videoService.videoModel(id);
        if (model != null) {
            Long start = 0L;
            Long end = 0L;
            if (time != null) {
