...
//    {
//        List<SysUserDept> depts = deptService.selectDeptList(dept);
//        List<TreeSelect> deptTress = deptService.buildDeptTreeSelect(depts));
//    }
    ...
    public AjaxResult add(@Validated @RequestBody SysDept dept) {
        ...
        int res = deptService.insertDept(dept);
        if (res > 0) {
    ...
    public AjaxResult remove(@PathVariable Long deptId) {
        ...
        int res = deptService.deleteDeptById(deptId);
        if (res > 0) {
            redis.delCacheObjectList(redis.sys_dept);
            List<SysDept> dps = deptService.setAllDeptChild();
            redis.addCacheObjectList(redis.sys_dept, deptService.selectDeptList(new SysDept()));
