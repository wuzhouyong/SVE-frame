...
import com.ruoyi.project.common.utils.MyHeaderFooter;
import com.ruoyi.project.common.utils.Watermark;
...
public class ExportPdfController extends BaseController {
    ...
    @PostMapping("/pdf")
    public AjaxResult pdf() {
        try {
            Document document = new Document(PageSize.A4); // 指定页面大小为A4
            //generatePDF(document);
            File file = new File("F:\\IdeaProjects\\SVE-frameNew\\PDFDemo.pdf");// 2.建立一个书写器(Writer)与document对象关联
            file.createNewFile();
            ...
            new ExportPdfController().generatePDF(document);
            // 5.关闭文档
            document.close();
        } catch (Exception ex) {
            System.out.print(ex);
