...
import com.ruoyi.project.youyan.mapper.DataSearchMapper;
...
public class DataSearchImpl implements DataSearchService {
    ...
    public DataSearchResult getReal(DataSearchParam entity) {
        ...
        List<DataSearchColEntity> cols = dataSearchMapper.getDataCol(entity.getSourceId(), entity.getOutletId());
    ...
    public ResultEntity getDaySummary(DataSearchParam entity) {
        /*Date startTime = DateStringUtils.stringToDate("yyyy-MM-dd", entity.getStartTime());
        Calendar calendar = new GregorianCalendar();
        calendar.setTime(startTime);
        calendar.add(Calendar.DAY_OF_MONTH, 1);
        Date endTime = calendar.getTime();
    ...
    public List<DataSearchColEntity> getDataCol(String sourceId, String outletId) {
        return dataSearchMapper.getDataCol(sourceId, outletId);
    }

    @Override
    public List<FacilityRelationEntity> selectFacility(String sourceId) {
        return dataSearchMapper.selectFacility(sourceId);
    }
    ...
    @Override  // 油烟近30天传输率曲线图数据-Minute
    ...
    @Override  // 获取油烟近30天传输率图总数据,分钟,小时,天 数据
