...
@RestController
@RequestMapping("/youyan/video")
public class YYVideoController extends BaseController {

    @Autowired
    private YYVideoService videoService;
    ...
    public TableDataInfo videotype() {
        List<YYNameIdEntity> list = videoService.selectVideoType();
        return getDataTable(list);
    }

    @PostMapping("/list")
    @DataScope(regionAlias = "s")
    public TableDataInfo list(@RequestBody YYVideoEntity entity) {
        startPage();
        ...
        List<YYVideoEntity> list = videoService.selectVideo(entity);
        return getDataTable(list);
    }

    @PreAuthorize("@ss.hasPermi('youyan:video:add')")
    @Log(title = "视频新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody YYVideoEntity entity) {
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
    ...
    @PreAuthorize("@ss.hasPermi('youyan:video:edit')")
    @Log(title = "视频编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody YYVideoEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(videoService.updateVideo(entity));
    }

    @PreAuthorize("@ss.hasPermi('youyan:video:del')")
    ...
    public ResultStr getVideoWS(String id, Date time) {
        YYVideoEntity model = videoService.videoModel(id);
        if (model != null) {
            Long start = 0L;
            Long end = 0L;
            if (time != null) {
