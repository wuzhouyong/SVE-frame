...
@RestController
@RequestMapping("/youyan/checkpoint")
public class YYCheckPointController extends BaseController {

    @Autowired
    private YYCheckPointService checkPointService;


    @PreAuthorize("@ss.hasPermi('youyan:checkpoint:add')")
    @Log(title = "监测点新增", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public AjaxResult add(@RequestBody YYCheckPointEntity entity) {
    ...
    public TableDataInfo list(@RequestBody YYCheckPointEntity entity) {
        startPage();
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<YYCheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        return getDataTable(list);
    }

    @PreAuthorize("@ss.hasPermi('youyan:checkpoint:edit')")
    @Log(title = "监测点编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody YYCheckPointEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(checkPointService.updateCheckPoint(entity));
    }

    @PreAuthorize("@ss.hasPermi('youyan:checkpoint:del')")
    ...
    public TableDataInfo cpList(@PathVariable String sourceId) {
        ...
        startPage();
        List<YYNameIdEntity> list = checkPointService.selectCPList(sourceId);
        return getDataTable(list);
    }
