...
@RestController
@RequestMapping("/qixiu/checkpoint")
public class QXCheckPointController extends BaseController {
    ...
    @PostMapping("/add")
    public AjaxResult add(@RequestBody QXCheckPointEntity entity) {
    ...
    public TableDataInfo list(@RequestBody QXCheckPointEntity entity) {
        startPage();
        if (!StringUtils.isEmpty(SecurityUtils.getLoginUser().getUser().getSourceId())) {
            entity.setSourceId(SecurityUtils.getLoginUser().getUser().getSourceId());
        }
        List<QXCheckPointEntity> list = checkPointService.selectCheckPoint(entity);
        return getDataTable(list);
    }

    @PreAuthorize("@ss.hasPermi('qixiu:checkpoint:edit')")
    @Log(title = "监测点编辑", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public AjaxResult edit(@RequestBody QXCheckPointEntity entity) {
        entity.setUpdatedBy(SecurityUtils.getUsername());
        return toAjax(checkPointService.updateCheckPoint(entity));
    }

    @PreAuthorize("@ss.hasPermi('qixiu:checkpoint:del')")
    ...
    public TableDataInfo cpList(@PathVariable String sourceId) {
        ...
        startPage();
        List<QXNameIdEntity> list = checkPointService.selectCPList(sourceId);
        return getDataTable(list);
    }
