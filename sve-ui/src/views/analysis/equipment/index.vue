  ...
  <div class="task-wk equipment-style">
    ...
    <el-table
      ...
      <el-table-column label="合计" align="center">
        <el-table-column label="安装数量" prop="town_cnt" width="140" align="center"></el-table-column>
      </el-table-column>
      <template v-for="(val,key,index) in col">
        <el-table-column :label="val" :key="index" align="center">
          <el-table-column label="安装数量" :prop="key+'_cnt'" width="100" align="center">
          ...
          <el-table-column label="设备传输率(平均)" width="130" align="center">
            <template slot-scope="scope">
              {{ scope.row[key + '_trans'] || '-' }}
            </template>
          </el-table-column>
        </el-table-column>
      </template>
...
<script>
import {equipBrandList} from '@/api/currency'
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true;
      equipBrandList(this.sysName).then(res => {
        if (res.code === 200) {
