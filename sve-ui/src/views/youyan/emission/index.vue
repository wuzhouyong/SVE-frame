...
import {yyTownEmission, yyEnterpriseEmission} from "@/api/youyan";
...
export default {
  ...
  watch: {
    townName: {
      handler(v) {
        yyEnterpriseEmission({townName: v}).then(res => {
          if (res.code === 200) {
  ...
  data() {
    ...
    return {
      townData: {
        columns: ["town_name", "max_amt"],
        rows: []
      },
      emissionseCity: {
        columns: ['datadate', 'max_amt'],
        rows: []
      },
      emissionseTown: {
        columns: ['datadate', 'max_amt'],
  ...
  methods: {
    ...
    getCityEmission(v = {}) {
      yyEnterpriseEmission(v).then(res => {
        if (res.code === 200) {
