          ...
          <el-col :lg="24" :sm="24">
            <el-form-item label="关联任务">
              <el-checkbox-group v-model="form.tagIds">
                <el-checkbox v-for="i in analysis" :key="i.id" :label="i.id">{{i.name}}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
          </el-col>
          <el-col :lg="24" :sm="24">
    ...
    <!-- <el-dialog :title="title" :visible.sync="openDataScope" width="500px" append-to-body>
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="权限范围">
          <el-select v-model="form.dataScope" :disabled="true">
            <el-option
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitDataScope" v-preventClick>确 定</el-button>
  ...
  methods: {
    ...
    getList() {
      ...
      listRole(this.addDateRange(this.queryParams, this.dateRange)).then(
        (response) => {
          let oRes = response.rows;
          this.roleList = oRes.sort((i, a) => {
            return i.roleSort - a.roleSort;
          });
      ...
      if (typeof roleId === "number") {
        if (roleId !== 2) {
          ...
          getRole(roleId).then((response) => {
            this.form = { ...{ tagIds: [] }, ...response.data };
            this.open = true;
            this.title = "修改角色";
          });
        } else {
          this.msgError("系统内置角色禁止操作");
        }
      } else {
        let set = new Set(roleId);
        if (!set.has(2)) {
          this.$nextTick(() => {
            this.getRoleMenuTreeselect(roleId);
          });
