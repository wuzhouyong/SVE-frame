<template>
  <div class="task-wk">
    ...
    <el-form :model="queryForm" ref="queryForm" :inline="true" class="outwater-serchForm">
      ...
      <el-form-item>
        ...
        <el-button
          ...
          size="mini"
          @click="ondutyHandle"
          v-hasPermi="['warn:substation:handle']"
          :disabled="!single"
        >分局处理
        </el-button>
      </el-form-item>
    ...
    >
      ...
      <el-table-column prop="startDate" label="异常时间" width="100">
    ...
    />
    <!-- 异常处理 -->
    <el-dialog title="分局处理" width="40%" :visible.sync="visible" custom-class="handleWarnDialog">
      <handleForm
...
export default {
  ...
  created () {
    ...
    this.getDicts("psh_proc_state").then((response) => {
  ...
  methods: {
    /** 获取列表 **/
    getList (v = {}) {
      warnList(v, this.activeName, this.urlParameter(this.page)).then(
        (res) => {
          if (res.code === 200) {
