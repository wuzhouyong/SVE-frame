<template>
  <div class="task-wk">
    ...
    <el-form :model="queryForm" ref="queryForm" :inline="true" class="voc-serchForm">
      ...
      <el-form-item label="排放风机名称">
        <el-input
          v-model="queryForm.facilityName"
          placeholder="请输入排放风机名称"
          clearable
          size="mini"
          style="width: 180px"
          @keyup.enter.native="handleQuery"
    ...
    >
      <el-table-column type="expand">
        <template slot-scope="props">
          <div>
            ...
            >排放风机关联排放口：{{props.row.pidName?props.row.pidName.join('、'):'-'}}</p>
      ...
      <el-table-column prop="facilityName" label="排放风机名称"></el-table-column>
...
<script>
import { EmissionList } from "@/api/voc";
export default {
  data() {
    return {
  ...
  methods: {
    getList() {
      EmissionList(this.queryForm).then((res) => {
        if (res.code === 200) {
