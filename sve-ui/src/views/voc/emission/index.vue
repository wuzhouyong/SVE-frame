        ...
        <el-col :lg="12" :md="12" :sm="24">
          <div class="charts_right">
            ...
            <div class="charts_right_btm">
              <div class="psh-kanban-title">
                <i class="el-icon-s-data"></i>
                <p>{{townName ? townName : '镇街' }}近半年({{ oType === 'amt' ? '排放量' : oType === 'max_amt' ? '最高浓度' : '平均浓度' }})统计</p>
              </div>
              <ve-histogram
                :data="emissionseTown"
                height="100%"
...
import {vocTownEmission, vocEnterpriseEmission} from "@/api/voc";
...
export default {
  ...
  watch: {
    townName: {
      handler(v) {
        vocEnterpriseEmission({townName: v}).then(res => {
          if (res.code === 200) {
  ...
  methods: {
    ...
    getCityEmission(v = {}) {
      vocEnterpriseEmission(v).then(res => {
        if (res.code === 200) {
