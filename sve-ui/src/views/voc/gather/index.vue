<template>
  <div class="task-wk">
    ...
    <el-form :model="queryForm" ref="queryForm" :inline="true" class="voc-serchForm">
      ...
      <el-form-item label="收集风机名称">
        <el-input
          v-model="queryForm.facilityName"
          placeholder="请输入收集风机名称"
          clearable
          size="mini"
          style="width: 180px"
          @keyup.enter.native="handleQuery"
    ...
    >
      <el-table-column type="expand">
        <template slot-scope="props">
          <div>
            ...
            >收集风机关联生产线：{{props.row.pidName?props.row.pidName.join('、'):'-'}}</p>
            <div class="el-show-style">
              <el-card
                class="box-card el-box-card"
                v-for="o in props.row.entity"
                :key="o.facilityId"
              >
      ...
      <el-table-column prop="facilityName" label="收集风机名称"></el-table-column>
...
<script>
import { collectBlowerList } from "@/api/voc";
export default {
  data() {
    return {
  ...
  methods: {
    getList() {
      collectBlowerList(this.queryForm).then((res) => {
        if (res.code === 200) {
