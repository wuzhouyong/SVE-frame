<template>
  ...
  >
    ...
    <el-form-item label="排放口" prop="outletId">
      <el-select v-model="form.outletId" placeholder="请选择排放口" style="width:100%">
        <el-option :label="i.outletName" :value="i.outletId" v-for="i in outList" :key="i.outletId"></el-option>
      </el-select>
    </el-form-item>
...
export default {
  ...
  watch: {
    values() {
      ...
      if (this.form.sourceId) {
        ...
        this.getoutletList(this.form.sourceId);
      }
  ...
  methods: {
    getoutletList(v) {
      ...
      outletList(oVal, this.urlParameter(this.page)).then((res) => {
