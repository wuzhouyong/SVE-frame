<template>
  ...
  >
    <el-form-item label="视频名称" prop="videoDesc">
      <el-input v-model="form.videoDesc" placeholder="请输入视频名称" size="mini">
        <!-- <template slot="prepend">{{form.eleTypeId}}</template> -->
      </el-input>
    </el-form-item>
    <el-form-item label="关联排放口" prop="outletId">
      <el-select v-model="form.outletId" placeholder="关联排放口" style="width:100%" size="mini">
        <el-option
          :label="i.outletName"
          :value="i.outletId"
          v-for="i in outletList"
          :key="i.outletId"
        ></el-option>
      </el-select>
    </el-form-item>
    <el-form-item label="视频类型" prop="videoType">
      <el-select v-model="form.videoType" placeholder="视频类型" style="width:100%" @change="onChange" size="mini">
        <el-option
          :label="i.dictLabel"
          :value="i.dictValue"
    ...
    <el-form-item label="视频地址" prop="videoAddress">
      <el-input v-model="form.videoAddress" autocomplete="off" placeholder="视频地址" size="mini"></el-input>
    </el-form-item>
    <el-form-item
...
export default {
  ...
  created() {
    ...
    list("qixiu", {}, "pageNum=1&pageSize=20").then(res => {
      if (res.code === 200) {
        this.outletList = res.rows;
      }
    });
  },
  methods: {
    ...
    onChange(v) {
      this.$set(this.form,'appsecret',null);
      this.$set(this.form,'appkey',null);
