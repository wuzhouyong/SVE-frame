<template>
  <div class="base-list">
    ...
    >
      ...
      <el-table-column prop="facilityName" width="500" label="风机名称"></el-table-column>
...
export default {
  ...
  methods: {
    queryCollection() {
      this.loading = true;
      FactorType.queryCollection().then((res) => {
        if (res.code == 200) {
    ...
    //添加操作
    handleAdd() {
      this.title = "新增VOCs收集风机";
      this.editValues = {};
      this.visible = true;
    },
    //修改操作
    handleUpdate() {
      this.title = "编辑VOCs收集风机";
      this.visible = true;
    ...
    handleDelete() {
      this.$confirm("是否确认删除选中数据数据项?", "警告", {
        ...
        .then(() => {
          return FactorType.removrCollection(this.ids);
        })
        .then(() => {
          this.queryCollection();
